                        ...
                        @foreach ($lists as $key=>$value)
                            <div class="tab-pane fade {{ ($loop->index == 0)? 'show active': '' }}" id="v-pills-{{$key}}" role="tabpanel" aria-labelledby="v-pills-{{$key}}-tab">
                                <div>
                                    ...
                                    <form action="{{ route('dashboard.settings.update',$key) }}" method="post" enctype="multipart/form-data">
                                        @isset($value['form']['group'])
                                            @includeIf('dashboard.pages.settings.inc.more',['data'=>$value['form']['group'],'group_by'=>$key])
                                        @else
                                            @isset($value['form']['lang'])
                                                @includeIf('dashboard.pages.settings.inc.lang',['form'=>$value['form'],'group_by'=>$key])
                                            @endisset
                                            @includeIf('dashboard.pages.settings.inc.inputs',['form'=>$value['form']])
                                        @endisset
