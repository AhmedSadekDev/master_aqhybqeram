...
<div style=" margin-bottom: 14px; position: relative; text-align: left; ">
    <a type="button" class="btn btn-primary my-action"  href="{{route('dashboard.categories.create')}}">@lang('Create new category')</a>
</div>
                        ...
                        <tbody>
                            @foreach ($lists as $list)
                                <tr>
                                    ...
                                    <td>
                                        <h5 class="text-truncate font-size-14"><a href="{{route('dashboard.categories.edit' , $list->id)}}" class="text-dark">{{ $list->name ?? '' }}</a></h5>
                                    </td>
                                    <td>
                                        <a href="{{ route('dashboard.categories.change-status',$list->id) }}">
                                            {!! $list->showStatus() !!}
                                        </a>
                                    </td>
                                    <td>
                                    ...
                                    <td style="display: inline-flex;">
                                        <a style="margin-left: 5px;" 
                                            ...
                                            href="{{ route('dashboard.categories.edit',$list->id) }}">
                                            <i class="bx bx-pencil"></i>
                                        </a>
                                        {!! action_table_delete(route('dashboard.categories.destroy',$list->id),$list->id) !!}
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
